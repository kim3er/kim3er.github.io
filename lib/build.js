...
export default async function init() {
	...
	async function grabPostData() {
		...
		for (const file of files) {
			...
			postsByYear[year].push(data);

			postCount++;
		}

		// postsByYear.sort(sortStringsAsNumberReverse);
	...
	async function renderArchive(posts) {
		...
		while (i < pagesOfPosts) {
			...
			log('ARCHIVE', `Written page ${chalk.cyan(i)} of ${chalk.cyan(pagesOfPosts)}`);
		}
	}
	...
	async function buildTemplates() {
		...
		const { postsByYear, categories } = await grabPostData();

		for (const year in postsByYear) {
			await renderPosts(postsByYear[year]);
		}

		await renderArchive(postsByYear);
